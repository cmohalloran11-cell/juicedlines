...
def test_pp_league_nflp_is_excluded_because_no_engine_projects_preseason():
    """NFLP carries the "nfl" substring, so it has to be excluded explicitly the same way the
    period/season-long sub-leagues are — otherwise preseason props reach the board and get
    projected by the regular-season model."""
    assert pullers._sport_from_pp_league("NFLP") == "other"
    assert pullers._sport_from_pp_league("nflp") == "other"
    assert pullers._sport_from_pp_league("NFL Preseason") == "other"
    assert pullers._sport_from_pp_league("NFL") == "NFL"
    assert pullers._sport_from_pp_league("nfl") == "NFL"


def test_fetch_prizepicks_drops_nflp_preseason_lines_entirely(monkeypatch):
    pullers._pp_result_cache.clear()
...
def test_fetch_prizepicks_still_keeps_plain_nfl_lines(monkeypatch):
    pullers._pp_result_cache.clear()
    ...
    lines, err = pullers.fetch_prizepicks()
    assert err is None
    assert len(lines) == 1 and lines[0]["sport"] == "NFL"
